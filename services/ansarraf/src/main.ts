...
const requiredProduction=['DATABASE_URL','CORS_ORIGIN','IDENTITY_SERVICE_URL','IDENTITY_ISSUER','IDENTITY_PUBLIC_KEY_B64','ANSARRAF_INTERNAL_TOKEN'];
...
if(pool){registerMarketRoutes(app,pool,marketData);registerTradingRoutes(app,pool);registerSettlementRoutes(app,pool);app.get('/api/v1/auth/me',{preHandler:requireAuth},async(request)=>{const auth=(request as typeof request&{auth:any}).auth;const id=await ensureCustomer(pool,auth);const r=await pool.query('SELECT id,identity_id,external_user_id,email,status FROM customers WHERE id=$1',[id]);return r.rows[0]?{user:r.rows[0]}:{error:'user_not_found'};});if(marketData)await marketData.start().catch(error=>app.log.error({error},'market data initial refresh failed'));}
const shutdown=async()=>{marketData?.stop();await app.close();await pool?.end();};
