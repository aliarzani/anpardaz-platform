...
export class AccountingOutboxWorker{
  ...
  private async postTrade(row:any){
    ...
    const buyerBase=await this.ensureAccount(Number(p.buyerCustomerId),base.symbol);const sellerBase=await this.ensureAccount(Number(p.sellerCustomerId),base.symbol);
    ...
    await this.postLedger({referenceType:'exchange_trade_base',referenceId:String(p.tradeId),idempotencyKey:row.idempotency_key+':base',description:'Exchange trade '+p.tradeId+' base settlement',entries:[{accountId:buyerBase,currency:base.symbol,direction:'credit',amount:String(p.quantity),metadata:{tradeId:p.tradeId,asset:'base'}},{accountId:sellerBase,currency:base.symbol,direction:'debit',amount:String(p.quantity),metadata:{tradeId:p.tradeId,asset:'base'}}]});
