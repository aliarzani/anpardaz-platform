...
if(pool){
 ...
 const news=new NewsService(new NewsRepository(pool));
 app.get('/api/v1/auth/me',{preHandler:requireAuth},async(request)=>{const auth=(request as typeof request&{auth:{sub:string;email:string;role:string}}).auth;const id=await ensurePlatformUser(pool,auth);const r=await pool.query<{id:string;identity_id:string;email:string|null;display_name:string|null;role:string;status:string}>('SELECT id,identity_id,email,display_name,role,status FROM platform_users WHERE id=$1',[id]);return r.rows[0]?{user:r.rows[0]}:{error:'user_not_found'};});
 app.get<{Querystring:{page?:string;limit?:string}}>('/api/v1/news',async(request,reply)=>{try{return await news.list(Number.parseInt(request.query.page??'1',10)||1,Number.parseInt(request.query.limit??'20',10)||20);}catch(e){request.log.error(e);return reply.code(503).send({error:'database_unavailable'});}});
 registerContentRoutes(app,pool);
 registerForumRoutes(app,pool);
