...
import {requireAuth,type AuthClaims} from '../auth.js';import {hasPermission} from '../permissions.js';
...
export function registerMarketDataRoutes(app:FastifyInstance,pool:Pool){
 ...
 app.get('/api/v1/market-data/sources',{preHandler:requireAuth},async(req,reply)=>{if(!(await hasPermission(pool,r(req).auth,'service_health.read')))return deny(reply);return{sources:(await pool.query('SELECT s.*,h.status,h.last_success_at,h.last_failure_at,h.consecutive_failures,h.last_error FROM market_data_sources s LEFT JOIN market_data_health h ON h.source_id=s.id ORDER BY s.priority,s.name')).rows};});
