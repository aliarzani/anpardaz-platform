              ...
              {items.map(c=>{
                ...
                return (
                  <div key={c.id} style={{position:"relative",marginBottom:4}}>
                    <button onClick={()=>onOpenConv(c)} style={{
                      width:"100%",display:"block",padding:"13px 14px",
                      background:"var(--ah-card)",border:"1px solid var(--ah-border)",
                      borderRadius:14,cursor:"pointer",textAlign:"right",transition:"all 0.13s",
...
export default function AnHooshScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <>
      <style>{`
        .ah-root{
          --ah-bg:#09090f; --ah-surface:#111118; --ah-card:#15151e;
          --ah-input:#1b1b27; --ah-border:rgba(255,255,255,0.07);
          --ah-border-acc:rgba(139,92,246,0.18);
          --ah-text:#edecfa; --ah-muted:rgba(237,236,250,0.38); --ah-accent:#8b5cf6;
        }
        .light-theme .ah-root{
